...
class RegistrationCon extends GetxController{
  ...
  TextEditingController whatsappCountryCodeCon = TextEditingController(text: "+91");
  ...
  Future<void> onRegister()async
  {
    ...
    try{
      if (fullName != "") {
        if(((countryCode.length>=1) && (countryCode.contains("+"))) && ((whatsappCountryCode.length>=1) && (whatsappCountryCode.contains("+")))){
              ...
              if (pincode.length >= 4 && pincode.length <= 8) {
                if(visitingCard!=""){
                  if (isAcceptedTerms.value) {
                   ...
                   user=UserDetail(phoneNumber: "$countryCode$phno", pincode: pincode, visitingCard: File(visitingCard), email: email, name: fullName, whatsappNumber: "$whatsappCountryCode$whatsappPhone");
